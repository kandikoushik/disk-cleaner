...
[![Version](https://img.shields.io/badge/Version-v4.0.0_Live-10b981.svg)](CHANGELOG.md)
...
**Disk Cleaner** is the **best open-source multiplatform disk cleanup, storage optimizer, and system utility**. Engineered for **macOS, Windows, iOS, and Android**, it delivers blazing-fast parallel directory sizing, content-hash duplicate detection, deep app uninstallation, Menu Bar status extra, custom JSON plugin targets, Docker/Podman volume purges, and system maintenance — all with **zero background telemetry, zero tracking, and 100% privacy**.

---

## 🚀 Downloads & Quick Install

- 🍏 **macOS Universal Binary (v4.0.0 DMG)**: [<img src="https://img.shields.io/badge/Download_DiskCleaner--4.0.dmg-10b981?style=for-the-badge&logo=apple&logoColor=white">](https://github.com/kandikoushik/disk-cleaner/releases/download/v4.0.0/DiskCleaner-4.0.dmg)
...
## 🌟 Major Release v4.0.0 Features

- 🚀 **Menu Bar Status Extra**: Native macOS menu bar status extra widget with live disk space monitoring and quick-clean shortcuts.
- 🔌 **Dynamic Custom JSON Plugin Target Engine**: Loads user custom cleanup rules from `~/.config/disk-cleaner/plugins/*.json`.
- 🐳 **Docker & Podman Volume Purger**: Target rules for dangling volumes, stopped containers, and build caches (`docker system prune`).
- 📱 **Xcode Simulator & Device Wiper**: Complete Simulator cache, Swift preview render, and iOS DeviceSupport wiper.
- 🍺 **Homebrew Cask Orphan Cleaner**: Auto-purge orphaned Homebrew cask archives and downloads.
- 🪟 **Windows WinSxS Store Analyzer**: Added Component Store DISM analysis to `DiskCleaner.exe`.
- ⏰ **Background Auto-Clean Daemon**: LaunchAgent script for weekly automated `.safe` tier cleaning (`scripts/install_auto_clean_daemon.sh`).
...
| :--- | :--- | :--- | :---: | :--- |
| **macOS** | `DiskCleaner-4.0.dmg` / `Disk Cleaner.app` | Pure Swift 5.10 & SwiftUI | 🟢 **Stable v4.0** | ~30 Smart targets, TaskGroup parallel sizing, 3-pass SHA-256 duplicate hashing, Liquid Glass UI, LaunchServices & Spotlight maintenance, Menu Bar extra. |
...
### Install Auto-Clean Daemon
```bash
./scripts/install_auto_clean_daemon.sh
```
